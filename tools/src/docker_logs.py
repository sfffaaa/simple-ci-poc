...
def _compose_partial_container_name(container_names, node_type, target):
    ...
    if node_type == "relay":
        ...
        data = [
            name for name in container_names if any([f in name for f in partial_names])
        ]
        if len(data) > 0:
            return data[0]
        raise ValueError(f"No parachain container found in {container_names}")
    elif node_type == "3rd":
        filter_name = ["-relaychain-"] + [
            f"-parachain-{s}-" for s in PEAQ_PARACHAIN_IDS
        ]
        data = [
            name
            for name in container_names
            if not any([f in name for f in filter_name])
        ]
        if len(data) > 0:
            return data[0]
        else:
...
def get_docker_service(in_type, target=0):
    ...
    container_name = _compose_partial_container_name(
        [container.name for container in my_docker.container.list()], in_type, target
    )
