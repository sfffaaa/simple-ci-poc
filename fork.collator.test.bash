...
cd "${PEAQ_NETWORK_NODE_FOLDER}" || { echo_report "${REPORT_PATH}" "cannot cd to ${PEAQ_NETWORK_NODE_FOLDER}"; exit 1; }
...
if [[ $CHAIN == "peaq-dev" || $CHAIN == "all" ]]; then
	...
	if ! SURI=$(get_code_key "${CHAIN_PWD}" "peaq-dev"); then
	...
	if ! execute_another_collator_node "${SURI}" "peaq-dev" "${fork_folder}" "${OUT_FOLDER_PATH}"; then
	...
	if ! execute_runtime_upgrade_only "peaq-dev" "${PEAQ_DEV_RUNTIME_MODULE_PATH}" "${OUT_FOLDER_PATH}"; then
		echo_report "${REPORT_PATH}" "cannot upgrade"
		ERROR_HAPPENED=1
	fi
	
	if ! execute_runtime_upgrade_pytest "peaq-dev" "test_did_add" "${PEAQ_DEV_RUNTIME_MODULE_PATH}" "${OUT_FOLDER_PATH}"; then
...
if [[ $CHAIN == "krest" || $CHAIN == "all" ]]; then
	...
	if ! SURI=$(get_code_key "${CHAIN_PWD}" "krest"); then
		echo_report "${REPORT_PATH}" "cannot get the key"
		ERROR_HAPPENED=1
		exit 1
	fi
	...
	if ! execute_another_collator_node "${SURI}" "krest" "${fork_folder}" "${OUT_FOLDER_PATH}"; then
	...
	if ! execute_runtime_upgrade_only "krest" "${KREST_RUNTIME_MODULE_PATH}" "${OUT_FOLDER_PATH}"; then
		echo_report "${REPORT_PATH}" "cannot upgrade"
		ERROR_HAPPENED=1
	fi
	
	if ! execute_runtime_upgrade_pytest "krest" "test_did_add" "${KREST_RUNTIME_MODULE_PATH}" "${OUT_FOLDER_PATH}"; then
		echo_report "${REPORT_PATH}" "cannot pass test"
		ERROR_HAPPENED=1
	fi
	reset_forked_collator
...
if [[ $CHAIN == "peaq" || $CHAIN == "all" ]]; then
	if ! SURI=$(get_code_key "${CHAIN_PWD}" "peaq"); then
		echo_report "${REPORT_PATH}" "cannot get the key"
		ERROR_HAPPENED=1
		exit 1
	fi
	...
	if ! execute_another_collator_node "${SURI}" "peaq" "${fork_folder}" "${OUT_FOLDER_PATH}"; then
	...
	if ! execute_runtime_upgrade_only "peaq" "${PEAQ_RUNTIME_MODULE_PATH}" "${OUT_FOLDER_PATH}"; then
		echo_report "${REPORT_PATH}" "cannot upgrade"
		ERROR_HAPPENED=1
	fi
	
	if ! execute_runtime_upgrade_pytest "peaq" "test_did_add" "${PEAQ_RUNTIME_MODULE_PATH}" "${OUT_FOLDER_PATH}"; then
		echo_report "${REPORT_PATH}" "cannot pass test"
		ERROR_HAPPENED=1
	fi
	reset_forked_collator
